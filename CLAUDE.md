...
**Order state machine** (`src/services/orderStateMachineService.js`) is the source of truth for order lifecycle — do not update `orders.status` directly in new code. It defines `ORDER_STATES` and `ALLOWED_TRANSITIONS`. (An earlier, narrower duplicate of this list, `src/utils/constants.js`'s `VALID_ORDER_STATUS_TRANSITIONS`, was unused dead code and has been removed.) Current full state list:
```
