...
- [x] C-2: Fix `TrackingScreen.js`/`ChatScreen.js` to read the token from SecureStore, not AsyncStorage — fixed & verified 2026-07-08: both files now `import { getToken } from '../services/api'` and call `getToken()` instead of `AsyncStorage.getItem('FLASH_TOKEN')`; repo-wide grep confirmed no other screen has this pattern. Verified live (no device/simulator available in this environment, so this is the strongest available substitute): registered a real test user via `POST /api/auth/user/register` against the running Docker backend, then ran the identical `io(BASE_URL, { auth: { token }, transports: ['websocket'] })` call from a Node script using the app's own `socket.io-client` dependency — got a real `connect` event and a clean `track_order` join, with no server-side rejection. Confirmed the `if (!token) return` guard is untouched, so a genuinely logged-out user still correctly no-ops. See commit for `fix(C-2)`.
- [x] C-3: Validate `item.quantity` is a positive integer in `Order.js` before pricing/stock logic — fixed & verified 2026-07-08: added a positive-integer check at the top of the item loop in `Order.js` (before both the stock adjustment and subtotal accumulation), removed the `parseInt(item.quantity) || 1` fallback that had silently coerced `quantity: 0` to `1` for pricing while using the raw `0` for the stock decrement, and added a `finalTotal <= 0` defense-in-depth check. Registered `Invalid quantity for` / `Order total must be positive` in `orderController.js`'s `CLIENT_ERROR_PREFIXES` so these surface as 400s, not 500s. Verified live against a rebuilt Docker backend with a real seeded product: **before fix** — the audit's exact two-line-item PoC (`quantity: 1` + `quantity: -1` on the same product) produced `subtotal: "0.00"` and left stock net-unchanged at `{"M": 20}` despite one real unit going out; a separate `quantity: 0` order produced `subtotal: "100.00"` (coerced to qty 1 for pricing) while stock also stayed at `{"M": 20}` (raw 0 used for the decrement) — both reproduced exactly as described. **After fix** — the identical negative-quantity payload now returns `400 {"error":"Invalid quantity for \"C3 Verify Test Shirt\": must be a positive integer"}` with stock unchanged; the identical zero-quantity payload returns the same 400; a legitimate `quantity: 2` order still succeeds (`201`, `subtotal: "200.00"`, stock correctly drops 20→18). See commit for `fix(C-3)`.
- [x] C-4: Add ownership check to `Driver.updateLocation`; add `requireApprovedDriver` to `/drivers/location`; reconcile driver-app `isOnline` on hydration — fixed & verified 2026-07-08.
  - **Server, ownership check**: `Driver.updateLocation` now runs `SELECT 1 FROM orders WHERE id=$1 AND driver_id=$2` and drops (nulls out) `orderId` on mismatch before it's used for persistence or broadcast. Verified live against a rebuilt Docker backend with two real driver accounts and a real order: **before** — Driver B (not assigned to Order X, owned by Driver A) POSTed a location tagged with Order X, got `200 success:true`, and a socket listener joined into Order X's tracking room *as the real customer* received the broadcast (`driverId` = Driver B, live coordinates) — the leak, reproduced exactly. **After** — the identical request still returns `200` (the driver's own position still updates), but the listener received nothing, and `driver_locations` rows written on the 5th ping show `order_id: NULL`, not the mismatched order. A sanity check confirmed the legitimate owner's (Driver A's) ping still broadcasts correctly to the customer.
  - **Server, `requireApprovedDriver` gate**: added to the `/drivers/location` route. Verified live: a driver left in `pending_documents` (registered but never approved — uses the token issued at registration, since login itself is gated behind `approved` status) got `200` before the fix and `403 {"error":"Please upload your required documents."}` after.
  - **Client, `isOnline` hydration reconciliation**: `DriverContext.js` no longer leaves `isOnline` hardcoded to `false` on every cold start. On hydration it now checks both `Location.hasStartedLocationUpdatesAsync(BACKGROUND_LOCATION_TASK)` (real OS task state) and a fresh `GET /drivers/me` (server truth), and resolves any disagreement down to "not online": stops an orphaned OS task if one is running, and corrects a stale server-side `is_online=true` via `POST /drivers/online {online:false}` if the OS task isn't actually running. No physical device/simulator is available in this environment (expo-location is a native module with no JS-only fallback, so the file can't even be `require()`d outside a real device) — verified as far as possible short of that: `npx expo lint` passes clean on the change; the exact reconciliation branch was extracted verbatim and run against all 4 real-world (OS-task-state × server-state) combinations in a controlled Node trace, all 4 correct; and the live `GET /drivers/me` call the logic depends on was independently confirmed to return real, current `is_online` data from a live toggle. The one piece that genuinely cannot be verified here is `Location.hasStartedLocationUpdatesAsync()` itself executing on a real OS. See commit for `fix(C-4)`.

**High**
- [x] H-1: Decide a real coverage plan (lower threshold honestly + ratchet up, or write real tests for the state machine/socket auth first) — resolved 2026-07-08 as a deliberate, documented decision (not a silent lowering): wrote genuine unit tests for the two files the audit flagged as most correctness-critical and least covered — `orderStateMachineService.js` (`tests/unit/orderStateMachine.test.js`, 23 tests: illegal-transition rejection, driver-ownership checks, the post-pickup cash-payment gate, wallet release-on-completion exactly once, and `assignDriver`'s double-assignment race prevention including the trusted-driver exclusivity window — went from ~4% to **87%/72%/73%** statements/branches/functions) and `socket/socketServer.js` (`tests/unit/socketServer.test.js`, 18 tests: the JWT auth middleware, the revoked-token check, per-role event authorization for `track_order`/`driver_location_update`/`join_driver_pool`, and the HIGH-9 periodic re-verification-and-disconnect behavior — went from **0%** to **67%/54%/55%**). One genuine bug found and fixed *in the new test itself* along the way: a shared module-level authorization cache inside `socketServer.js` caused cross-test pollution when two tests reused the same socket id — fixed by giving every mock socket a unique id, which also confirmed the real module's 30-second authorization cache behaves as documented. `jest.config.js`'s `coverageThreshold` is now set to the honest, measured, post-test result (27%/16%/27%/12%, rounded down slightly for margin) — not 70% (never true), not the pre-this-work 21.85% (now stale) — documented in the config file itself as a deliberate baseline to ratchet up from, not a ceiling.
- [x] H-2: Fix `auth.test.js`'s `bcrypt`→`bcryptjs` mock and method names; add refresh-reuse test — fixed & verified 2026-07-08. Fixed the module-resolution blocker (`jest.mock('bcrypt')` → `bcryptjs`) and the wrong method names (`register`/`login` → `registerUser`/`loginUser`). That alone did **not** make the suite pass — a deeper, previously-invisible bug surfaced once it could actually run: the `generateToken` test's `jest.resetModules()` call, which ran first, invalidated every top-level `pool`/`bcrypt`/`jwt` reference the rest of the file depended on, so every mock setup in every later test silently applied to a stale, disconnected module instance. Fixed by moving that test to run last (no logic change, just position) rather than editing a test that already passed correctly in isolation. Along the way, fixed three more tests whose premises didn't match real controller behavior (not just the two H-2 named): the "terms not accepted" registration test asserted a check that doesn't exist anywhere in `registerUser` or its route validators (terms acceptance is a separate authenticated `POST /user/accept-terms` call, not a registration gate) — replaced with a real, currently-enforced rule (password must be ≥10 chars); the logout/jti test never configured `jwt.decode`'s mock return value, so the insert it claimed to verify never actually ran; the `resetPassword` "expired token" test simulated expiry via a row's `expires_at` field, but the real controller filters expiry in SQL (`WHERE expires_at > NOW()`) and never inspects that field in JS, so the mocked row was never actually rejected. Added 3 new tests for `refreshToken` (previously zero coverage): a happy-path rotation test, and the reuse-detection test H-2 specifically asked for — replaying an already-revoked refresh token — which asserts the controller both returns `401 "Session compromised..."` **and** issues the real `UPDATE refresh_tokens SET revoked_at = NOW() WHERE user_id = $1 AND revoked_at IS NULL` revoke-the-whole-family query with the correct `user_id`, not just that some error was thrown. All 14 tests in the file now pass for real (verified via `npx jest tests/unit/auth.test.js`, and via full `npm test` for regressions — one unrelated, pre-existing failure in `driverCommission.test.js` was found during this pass; confirmed via `git log`/`git status` that neither that test file nor its source has been touched in this engagement, so it's flagged separately below, not folded into this fix). Real coverage numbers for H-1 captured post-fix: 21.85%/9.14%/22.63%/8.02% (statements/branches/lines/functions), up from the audit's original 19.46%/7.22%/20.28%/5.34% now that this suite actually executes and contributes real coverage instead of failing at load time.
- [x] H-3: Scope OAuth audience checks per app (user vs driver) in both Apple and Google verification — fixed & verified 2026-07-08. **Google**: `verifyGoogleToken(idToken)` built its own accepted-audience list internally from all 4 configured client IDs (user+driver × Android+iOS) regardless of caller; now takes an `expectedAudiences` param, and each of `googleSignInUser`/`googleSignInDriver` passes only its own app's two client IDs. **Apple**: `verifyAppleToken(identityToken, clientId)` already received the caller's own `clientId`, but unconditionally added `process.env.APPLE_DRIVER_CLIENT_ID` to the accepted list regardless of which app was calling — so a driver-app-minted token was also accepted on the *user* sign-in endpoint (asymmetric: the reverse direction happened to already work correctly, since nothing added the user client ID to the driver path). Removed the unconditional driver-ID inclusion; only the caller's own `clientId` plus the shared web `APPLE_SERVICE_ID` (not flagged by the audit, left as-is) are accepted now.
  Real Google/Apple-signed tokens aren't obtainable in this environment, so both were verified by driving the real, unmodified `verifyGoogleToken`/`verifyAppleToken` exports against a synthetic-but-structurally-real network response — for Apple this included generating a throwaway RSA keypair, serving it as the JWKS response, and signing a real RS256 JWT with the matching private key, so the actual production signature-verification code path executes for real, not just the audience-check line. Before/after for both:
  - **Google, before fix**: a token minted for the user app (`aud=USER_ANDROID`) presented to the driver endpoint's audience list → accepted. A driver-app token presented to the user endpoint → accepted. Both wrong.
  - **Google, after fix**: both cross-app cases correctly rejected with `"Google token audience mismatch"`; both legitimate same-app cases still accepted.
  - **Apple, before fix**: user-app token on driver endpoint → correctly rejected (this direction was already fine). Driver-app token on user endpoint → **accepted** (the real bug, reproduced).
  - **Apple, after fix**: both cross-app cases correctly rejected with `"Apple token audience mismatch"`; both legitimate same-app cases still accepted, with a real RSA signature verified in every case.
  `npm test` re-run clean after this change (no new regressions; the one pre-existing unrelated failure is noted separately below).
- [x] H-4: Compute delivery fee server-side from coordinates, not client-supplied mall IDs — fixed & verified 2026-07-08. A discovery en route: the codebase already has a second, more sophisticated, entirely **unused** pricing engine (`backend/src/services/pricingService.js` — real haversine distance, R35 base + R8/km, peak-hour surcharge, R65–R500 bounds) that is never `require()`d anywhere in `src/`. Rather than silently switching the live pricing model to it (a real business decision affecting every order's fee and every driver's payout, not just a security fix), flagged the discovery and asked; founder chose to keep the existing R90/R180 two-tier pricing customers already see, gated by real distance instead of a client-supplied ID match. `Order.calculateDeliveryFee` now computes real haversine distance (reusing the existing shared `utils/helpers.calculateDistance`, not a new implementation) from the store's fixed, server-controlled pickup coordinate to the already-geofence-validated dropoff coordinate, and applies R90 within 5km, R180 beyond — a threshold I chose (documented as a single named constant, `NEARBY_DELIVERY_RADIUS_KM`) matching a "typical delivery distance" precedent already used elsewhere in this codebase (`pricingService.js`'s own 5km fallback), not something I want to represent as more precisely justified than that — flagging for the founder to adjust if it doesn't match real delivery economics.
  Verified live against a rebuilt Docker backend, before and after, with real coordinates (store: -33.884221,25.585318; a genuine ~2.9km-away point; a genuine ~15.6km-away point, both within the Nelson Mandela Bay service boundary):
  - **Before fix**: sending matching `pickup_mall_id`/`dropoff_mall_id` on the genuinely ~15.6km-away delivery produced `delivery_fee: "90.00"` and `driver_payout: "67.50"` — the exact manipulation, reproduced, with the driver-payout shrinkage the audit predicted.
  - **After fix**: the identical manipulation attempt now produces `delivery_fee: "180.00"`, `driver_payout: "135.00"` — real distance wins regardless of what mall IDs are sent. Also confirmed the *other* direction of the same bug is closed: mismatched mall IDs on the genuinely ~2.9km-away delivery still correctly produces `"90.00"` (previously this would have wrongly overcharged R180 despite being nearby). Two additional sanity checks with no mall IDs sent at all confirm the R90/R180 customer-facing pricing itself is unchanged — only the trust boundary moved.
  `tests/unit/orders.test.js`'s `Order.calculateDeliveryFee` suite tested the *old*, exploitable mall-ID-match behavior as the expected/correct behavior — updated to test the new coordinate-based behavior instead (same real coordinates as the live verification above), plus added a regression guard asserting mall IDs now have zero effect on the fee even when supplied. `npm test` re-run clean, no new regressions beyond the one pre-existing unrelated failure noted separately below.
- [x] H-5: Require step-up auth (password/OTP) before activating a new driver payout bank account — fixed & verified 2026-07-08. Implemented as password re-entry, not OTP: this is a deliberate, founder-approved choice (not a fallback pending something better) — there's no reliable SMS/OTP delivery infrastructure wired up in this codebase yet, and shipping a real (if simpler) protection now beats blocking the fix on building OTP delivery first. An OTP step remains a reasonable future upgrade. `driverController.saveBankAccount` now requires `password` in the request body, looks up the driver via `Driver.findById`, and verifies it with `bcrypt.compare` against `password_hash` before any Paystack call — wrong or missing password never reaches the payment provider. Also added a non-blocking push notification to the driver's own device on every successful change ("Your bank account for payouts was just updated..."), so a real driver gets a signal immediately instead of only noticing when a payout goes missing (reuses the existing `sendPushNotification` helper, no new infrastructure). Updated the driver app's bank screen and `services/api.js` wrapper to actually collect and send the password — a server-side-only fix would have silently broken this screen for every real user, since the request would always come back `400`.
  Verified live against a rebuilt Docker backend with a real approved driver account: missing password → `400 "Password confirmation is required to change payout details"`; wrong password → `401 "Incorrect password"`; correct password → passes the auth gate cleanly and proceeds to a different, unrelated error (`Paystack` not configured in this disposable test environment) — confirming the gate itself, not a downstream failure, is what blocked the first two cases. `npx expo lint` clean on the driver-app change. `npm test` re-run clean, no new regressions.
- [x] H-6: Gate `markCashFailed` behind `order.status === 'delivered'` + add rate limiting — fixed & verified 2026-07-08. Added the same `order.status !== 'delivered'` precondition `confirmCashReceived` already enforces (409 if not yet delivered), and added the shared `otpLimiter` (3/min) already used by its sibling routes `/cash/send-otp` and `/cash/confirm` — `/cash/fail` was the one cash route with no rate limit at all.
  **Scope note, stated plainly:** this closes the most blatant version of the abuse — calling `/cash/fail` before ever attempting delivery, with zero legitimate basis — not every version. A driver who has genuinely reached `delivered` status (i.e., actually handed over the goods) can still falsely claim the customer refused cash instead of running the honest OTP-confirmed flow; closing that fully would need something stronger than a status check (e.g. requiring OTP-verified failure too, or the audit's separately-listed Medium finding — tracking a per-driver fail-rate as a fraud signal). This fix is exactly what the audit specified and what was asked for; flagging the boundary rather than overclaiming coverage.
  Verified live against a rebuilt Docker backend, before/after, with a real cash order assigned to a real driver: **before fix** — the driver called `/cash/fail` on an order still in `driver_assigned` status (never delivered, no goods ever handed over) and got `200 {"success":true,"payment_status":"failed"}`, with the innocent customer's `cash_refusal_count` incremented to 1 — the exact abuse, reproduced. **After fix** — the identical premature call now returns `409 "Cash failure can only be reported after delivery"`; a sanity check confirmed the same order, once genuinely transitioned to `delivered`, still allows the legitimate call (`200`). Rate limiting confirmed separately: 4 rapid calls against a delivered order returned `200` once then `429 "Too many OTP requests..."` three times, consistent with the shared 3/min limiter. `npm test` re-run clean, no new regressions.
- [x] H-7: Add request timeouts to both mobile apps' `services/api.js` — fixed & verified 2026-07-08. Added a `fetchWithTimeout()` wrapper (20s, `AbortController`-based) to both `flash-driver-app/services/api.js` and `flash-user-app/services/api.js`, applied to all three `fetch()` call sites in each file's shared `request()` wrapper (the initial call, the refresh-token call, and the retry-after-refresh call) — every authenticated and unauthenticated request in both apps now has a ceiling, including cash-OTP and bank-account flows the audit specifically called out. On timeout, throws a clear user-facing message instead of hanging indefinitely.
  Neither app's native `fetch`/`AbortController` can run outside a device/RN runtime in this environment (and `services/api.js` also imports `expo-secure-store`, a native module, so the files can't be `require()`d directly either). Verified by extracting `fetchWithTimeout`'s logic verbatim (only the timeout constant shortened, 20s→500ms, for test speed) and driving it with Node's native `fetch`/`AbortController` — the same Fetch API surface React Native implements — against a real HTTP server that accepts the connection but never responds (simulating a dropped mobile connection): the call aborted at ~520ms with the exact configured error message, instead of hanging. A second check confirmed a normal fast request through the same wrapper completes normally with no interference. `npx expo lint` clean on the driver-app change (user app has no lint script, per this repo's existing setup).
- [x] H-8: Centralize `SESSION_EXPIRED` handling through one callback instead of 12 call sites — fixed & verified 2026-07-08. `flash-user-app/services/api.js` gained the exact `setSessionExpiredHandler`/`onSessionExpired` pattern already proven in `flash-driver-app/services/api.js` (a prior, separate fix — confirmed by reading it first rather than reinventing something different): `request()` now invokes the registered handler directly and unconditionally the instant a 401 is detected, before it even throws — independent of whether the caller's local `try/catch` swallows the error afterward, which is the dominant pattern in this app. `App.js` registers `FlashContext`'s existing `handleSessionExpired` (which already correctly reset `token`/`user`/`orders`/`cart` — it existed in the code already but was never actually being called except through the `ErrorUtils.setGlobalHandler` fallback, which only catches truly uncaught exceptions and so never fired in practice) via this new callback, keeping the `ErrorUtils` hook as a defensive fallback only.
  Also fixed the 9 real call sites (across `SavedCardsScreen.js` ×3, `SizingScreen.js`, `TrustedDriversScreen.js` ×2, `FeedScreen.js` ×2, `AddressScreen.js`, `SettingsScreen.js`) that displayed the raw `e.message` (or `err.message`) directly in an `Alert.alert('Error', ...)`, which would show the literal string "SESSION_EXPIRED" to the user on top of the redirect now firing — each now checks for that value and skips the alert, matching the guard pattern the 3 "silent swallow" sites (`CheckoutScreen.js`, `PaymentScreen.js`, `OrderStatusScreen.js`) already had (those three already correctly skipped the alert on `SESSION_EXPIRED`; their comments claiming "just redirect to login" were false before this fix — the redirect never actually happened — and are now true).
  Verified: since `services/api.js` imports `expo-secure-store` (a native module) and uses ESM syntax, it can't be `require()`d directly in this environment. Extracted the exact callback-registration logic added (verbatim) and traced it in a controlled Node test: a registered handler fires exactly once and before the `SESSION_EXPIRED` throw; no handler registered doesn't crash and still throws normally (safe before `App.js`'s `useEffect` has run); re-registering replaces rather than stacks handlers. All 3 cases pass. Full diff of every touched file re-read for correctness (no lint script exists for this app, per its own `package.json`, consistent with what CLAUDE.md already documents).
...
- [x] H-11: Same fix as C-4's client-side half — closed by the same `DriverContext.js` hydration reconciliation applied under C-4 (see C-4 entry above for verification detail); no separate change needed.
