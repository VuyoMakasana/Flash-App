...
const authenticate = async (req, res, next) => {
  ...
  // ADMIN PLATFORM PHASE 2: admin tokens have no refresh-token table the way
  // user/driver do (8h, single JWT, nothing else persisted about a live
  // session) — so "invalidate every other active session on password
  // change/reset" can't be done by revoking individual issued tokens (their
  // jti's are never stored anywhere until logout). Comparing this token's
  // own `iat` against admins.password_changed_at (set by
  // adminController.js's changePassword/resetPassword) achieves the same
  // real effect: any admin token issued before the most recent password
  // change stops working on its very next use, regardless of its remaining
  // 8h lifetime. `>=` (not `>`) on the whole-second comparison below is
  // deliberate — iat is whole-second-truncated by jsonwebtoken, so a token
  // minted in the same second the password was changed (the fresh
  // replacement token adminController.js returns from changePassword
  // itself, so the admin isn't logged out by the very request that changed
  // their password) must not be rejected by sub-second rounding.
  if (decoded.role === "admin" && verifiedWithAdminSecret) {
    try {
...
// ─── ADMIN — FORCE PASSWORD RESET GATE ────────────────────────────────────────
// Admin Platform Phase 2: a seeded/temporary admin password
// (admins.force_password_reset, migrate.js v34/v35) must not be usable for
// anything except actually changing it. Applied only to the specific admin
// routes that should be reachable in that state (login, change-password,
// logout are never gated by this — see adminRoutes.js) — every other
// admin-authenticated route refuses to serve the request while the flag is
// set, same "structural, not just documented" guarantee as the flag's own
// migration comment.
const requireAdminPasswordCurrent = async (req, res, next) => {
  try {
    const result = await pool.query(
      "SELECT force_password_reset FROM admins WHERE id = $1",
      [req.userId]
    );
