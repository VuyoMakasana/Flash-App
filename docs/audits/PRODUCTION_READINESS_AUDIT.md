...
- **HIGH, escalated from "unconfirmed" to confirmed — a Google Maps API key leaked in git history was never rotated.** The first report flagged `[REDACTED — see rotation note]` as permanently present in old commits and explicitly stated "not verified whether rotated." This session confirms directly: **that exact same key is still the live `GOOGLE_MAPS_API_KEY` value in `backend/.env` today.** It has never been rotated. Anyone who clones an old commit (this repo's history has not been rewritten, per policy) can extract and use this key against Flash's Google Cloud billing account, unless restricted by HTTP referrer/package name/SHA-1 fingerprint in Google Cloud Console — a setting only the founder can confirm, and which the first report already flagged as unconfirmed. **Recommend:** rotate this key in Google Cloud Console now, and confirm API restrictions are actually configured.
  - **[2026-09-05 remediation note]** The literal key value that previously appeared on this line has been redacted — it was live and readable in this tracked file (a separate, current exposure from the original git-history leak). **The key itself has not been rotated by this change** — it is still the real, active `GOOGLE_MAPS_API_KEY` in `backend/.env` and anywhere else it's deployed, until the founder rotates it in Google Cloud Console. Redacting this document does not resolve F-01; it only stops this file from being a second live copy of the secret.
