...
// Product images. Mirrors driverRoutes.js's own uploadPhoto config
// (memoryStorage, 10MB limit, image-only) — the same proven pattern, not a
// new one. multer's fileFilter only sees the client-declared mimetype
// (trivially spoofable); the controller's own magic-byte check via
// detectRealMimeType is the real gate.
...
// FLASH_STORE_ADMIN_DESIGN.md §5.3's RBAC table only grants inventory
// visibility ("Sees") to Owner, Store Manager, and Inventory Staff —
// different three roles than Orders' (Owner/Store Manager/Sales Staff),
// applied here literally. Finance/Marketing/Sales Staff get 403 on this
// whole route tree.
const INVENTORY_VISIBLE_ROLES = ['owner', 'store_manager', 'inventory_staff'];

router.use(authenticateStore, requireOwnStore, requireStorePasswordCurrent, requireStoreRole(...INVENTORY_VISIBLE_ROLES));
