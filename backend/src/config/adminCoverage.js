...
module.exports = {
  covered: {
    ...
    commission_rates: 'Phase 2b — real, browsable AdminJS resource, but READ-ONLY. Admins need to see which rate is active and which row produced the stamped commission on a given order (orders.commission_rate_id points here), so a dispute is reconstructable. new/edit/delete are disabled on purpose: FINANCIAL_DOMAIN_SPECIFICATION.md 2.4 requires a rate change to be an INSERT with the old row deactivated, never an in-place edit, because this table IS its own audit trail. Creating rates through the panel needs a real action with validation (a second active global violates the unique partial index), so it is deliberately deferred rather than exposed as a default form.',
